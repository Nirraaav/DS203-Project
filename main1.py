...
# Function to load and aggregate MFCC data based only on MFCC 2-5
def aggregate_mfcc_2_to_5(mfcc_data):
    """
    Aggregates MFCC data by selecting coefficients 2 to 5 (i.e., rows 1 to 4 in 0-indexed Python),
    and then calculating the mean, standard deviation, max, and min for these selected coefficients.
    """
    # Select rows corresponding to MFCC 2 to 5 (i.e., rows 1 to 4 in 0-indexed Python)
    mfcc_selected = mfcc_data[13:, :]  # Slice rows for MFCC 2, 3, 4, 5
...
for i in range(1, 116):  # Assuming 115 songs are numbered as '01-MFCC.csv', '02-MFCC.csv', ..., '115-MFCC.csv'
    ...
    aggregated_features = aggregate_mfcc_2_to_5(song_data.values)
...
output_df.to_csv('file_cluster_labels_mfcc_2_5.csv', index=False)
